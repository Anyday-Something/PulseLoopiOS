...
final class EventPersistenceSubscriber {
    ...
    private func persistSleepTimeline(start: Date, stages: [SleepStage]) {
        // Shared with `HealthImportService`, which persists nights read back out of Apple Health.
        SleepService.persistTimeline(start: start, stages: stages, context: context)
    }
}
